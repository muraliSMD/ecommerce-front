...
export default function ReviewsSection({ product, refetch }) {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-16">
        <h2 className="font-display font-bold text-3xl text-text-main mb-8 text-center">Customer Reviews</h2>

        <div className="flex justify-center mb-12">
            {/* Rating Summary */}
            <div className="w-full max-w-sm bg-bg-surface rounded-3xl p-8 shadow-sm border border-border-main text-center h-fit">
                <div className="text-5xl font-bold text-text-main mb-2">{product.averageRating?.toFixed(1) || 0}</div>
                <div className="flex justify-center gap-1 text-yellow-400 mb-2">
                     {[...Array(5)].map((_, i) => (
                        <FiStar key={i} className={i < Math.round(product.averageRating || 0) ? "fill-current" : "text-text-muted/30"} />
                     ))}
                </div>
                <p className="text-text-muted font-medium">{product.numReviews || 0} Reviews</p>
            </div>
        </div>
        ...
        <div className="space-y-6">
            {product.reviews?.length === 0 ? (
                <div className="text-center py-12 bg-bg-section/50 rounded-3xl">
                    <p className="text-text-muted">No reviews yet. Be the first to review this product!</p>
                </div>
            ) : (
                [...product.reviews].reverse().map((review) => (
                    <div key={review._id} className="bg-bg-surface rounded-3xl p-8 shadow-sm border border-border-main">
                        <div className="flex items-start justify-between mb-4">
                            <div className="flex items-center gap-3">
                                <div className="w-10 h-10 rounded-full bg-bg-section flex items-center justify-center text-text-muted">
                                    <FiUser />
                                </div>
                                <div>
                                    ...
                                    <div className="flex gap-1 text-yellow-400 text-xs mt-0.5">
                                        {[...Array(5)].map((_, i) => (
                                            <FiStar key={i} className={i < review.rating ? "fill-current" : "text-text-muted/30"} />
                                        ))}
                                    </div>
                                </div>
                            </div>
                         ...
                         {(review.images?.length > 0 || review.videos?.length > 0) && (
                            <div className="flex flex-wrap gap-3 mb-6">
                                {review.images?.map((img, idx) => (
                                    <div key={idx} className="relative w-24 h-24 rounded-2xl overflow-hidden border border-border-main cursor-pointer hover:opacity-90 transition-opacity">
                                        <Image src={img} alt="review" fill className="object-cover" />
                                    </div>
                                ))}
                                {review.videos?.map((vid, idx) => (
                                    <div key={idx} className="relative w-24 h-24 rounded-2xl overflow-hidden border border-border-main bg-black flex items-center justify-center cursor-pointer hover:opacity-90 transition-opacity">
